...
def auth_color(l):
    ...
    col = '#000'
    for i in l:
        if i == 'HillaryClinton' and col == '#000':
            col ='#00F'
        elif i == 'realDonaldTrump' and col == '#000':
            col ='#F00'
        elif (i == 'HillaryClinton' and col == '#F00') or (i == 'realDonaldTrump' and col == '#00F'):
            return '#0F0'
    return col
...
def make_nodes(ht_id_dict, cfg):
    ...
    for ht in ht_id_dict:
        ...
        #Colors: if Trump f00 if Clinton 00f both 0f0
        cur.execute('''SELECT handle
                        FROM tweets
                        WHERE id
                        IN (SELECT id
                            FROM contains
                            WHERE hname = '{0}'); '''.format(ht))
        fetchall = cur.fetchall()
        auths = lot2list(fetchall)
        col = auth_color(auths)
        # add to list
        l.append({'id': ht_id_dict[ht], 'label': ht, 'x': len(ht_alone_set), 'y' : len(ht_withothers_set), 'size': len(ht_appears_set), 'color': col})
